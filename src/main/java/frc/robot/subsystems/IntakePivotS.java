...
public class IntakePivotS extends SubsystemBase {
  ...
  public class IntakePivotConstants {
    ...
    // Pivot Offset from Zero degrees (when the code starts, it always resets the
    // angle to zero so this is neccesary
    ...
    private static TalonFXConfiguration configureMotor(TalonFXConfiguration config) {
      ...
      motionMagicConfigs.MotionMagicAcceleration = Units.degreesToRotations(20);
      ; // Target acceleration of 160 rps/s (0.5 seconds)
      motionMagicConfigs.MotionMagicJerk = Units.degreesToRotations(20);
  ...
  public static final TalonFX IntakePivotMotor = new TalonFX(IntakePivotConstants.INTAKE_PIVOT_MOTOR_CAN_ID,
  ...
  public Command moveToAngle(double someAngle) {
    ...
    return run(() -> IntakePivotMotor.setControl(m_request.withPosition(Units.degreesToRotations(someAngle))));
  }

  final MotionMagicVoltage m_request = new MotionMagicVoltage(Units.degreesToRotations(0))
      .withFeedForward(getArmFeedforward());
  ...
  // Methods:
  // Gets the output of the motor sensor, then converts it to the accurate radian
  // measure for the pivot
  ...
  public double getArmFeedforward() {
    return 0.95 * Math
        .cos(IntakePivotMotor.getRotorPosition().getValueAsDouble() / IntakePivotConstants.kArmGearRatio * 2 * Math.PI
            + IntakePivotConstants.kArmOffset);
  }
